...
| **Group Library ID** | Integer after `/groups/` in group URL (e.g. `https://www.zotero.org/groups/151202`) |

**Note:** Group libraries can be public (no API key needed for read access) or
private. Verified 2026-07-15: group `151202` is a live public group usable for
read-only testing without credentials — `Zotero('151202', 'group', None)` or
`https://api.zotero.org/groups/151202/items/top` both work. Older examples
elsewhere may reference stale/now-private group IDs; if a documented example
group 403s, treat it as the group having gone private, not an auth bug.
...
zot = Zotero('151202', 'group', os.environ['ZOTERO_API_KEY'])
```
