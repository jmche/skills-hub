...
If a listed MCP isn't connected, say so and fall back — in this order:

1. **Local database skills** (installed, no OAuth needed): `clinical-trials-database`,
   `pubmed-database`, `openfda-database` — these hit the public APIs directly and are the
   preferred fallback. `NCBI_API_KEY` is configured in `~/.shell_env` for higher PubMed rate limits.
2. `WebSearch` / `WebFetch` against the underlying public source
   (clinicaltrials.gov, pubmed.ncbi.nlm.nih.gov).

> Status (checked 2026-07-23): the `pubmed` and `clinical-trials` MCP servers are installed
> but **unauthenticated** — only their `authenticate` tools are exposed. Either run the OAuth
> flow once, or use the local skills above.
