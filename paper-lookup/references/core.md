...
> **Known instability (last observed 2026-07-15):** All endpoints below
> returned HTTP 500 `"OOM command not allowed when used memory >
> 'maxmemory'"` — a server-side Redis outage on CORE's end, not a client or
> auth error. If you hit a 500 with this message, don't retry-loop or assume
> your query/auth is wrong — fall back to Semantic Scholar, OpenAlex, or
> Crossref for the same lookup and mention to the user that CORE was
> unavailable.
