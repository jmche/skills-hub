...
> **`scripts/search_google_scholar.py` has been removed (2026-07-15) — Google
> blocks automated Scholar scraping and this is not fixable.** The query
> syntax/operators documented below are still valid if a human runs a search
> in a browser, but any command in this file that shells out to
> `search_google_scholar.py` no longer works. For programmatic/agent access,
> use PubMed, Crossref, or Semantic Scholar instead (via `search_pubmed.py`,
> `doi_to_bibtex.py`, or the `paper-lookup` skill).
