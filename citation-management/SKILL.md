...
> **`search_google_scholar.py` has been removed (2026-07-15).** Google
> actively blocks automated Scholar scraping (the `scholarly` library
> reliably fails with "Cannot Fetch from Google Scholar" / anti-bot errors)
> — this is a structural blocker, not a fixable endpoint bug. Every mention
> of `search_google_scholar.py` or `references/google_scholar_search.md`
> below is stale. Use `search_pubmed.py`, `doi_to_bibtex.py` (Crossref-backed),
> or the `paper-lookup`/`pubmed-database` skills instead — PubMed, Crossref,
> and Semantic Scholar cover the same ground without anti-bot blocking.
