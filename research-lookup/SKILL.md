...
**Do not run `scripts/research_lookup.py` with `uv run`** — unlike most other
skills in this library, this script has no PEP 723 inline dependency header,
so `uv run` fails with `ModuleNotFoundError: requests`. Always invoke it with
`python` / `python3` as shown in the command examples below.
