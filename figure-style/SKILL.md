...
The helpers live in `kernel.py` next to this file and are **not** auto-injected.
...
Requires `matplotlib`; `panel_crops`-based QA (§9.2) also needs `Pillow`.
...
After `fig.savefig(...)`, before you hand the figure to anyone:
...
for letter, box in panel_crops(fig).items():
    im.crop(box).save(f"panel_{letter}_crop.png")
```
Then open each `panel_<letter>_crop.png` with the **Read** tool and look at it.
