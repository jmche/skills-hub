...
Module top level is definition-only (functions, imports, literal constants);
everything that touches the network happens inside a function body.

Origin: adapted from the Claude Science `literature-review` skill. The host
SDK calls (`host.get_user_email`, `host.credentials.request`) have been
replaced with local equivalents — see `litrev_contact` and
...
SHELL_ENV_FILES = ("~/.shell_env", "~/.env")


def litrev_env(name: str) -> str | None:
    """Read ``name`` from the process environment, falling back to parsing
    ``~/.shell_env`` then ``~/.env``.

    The fallback exists because Claude Code's Bash tool runs a non-interactive,
    non-login shell seeded from a shell snapshot that exports only PATH, so a
    key defined in ``~/.shell_env`` is invisible here unless ``BASH_ENV`` is set
    in ``~/.claude/settings.json`` AND the assignment carries ``export``.
    Never log or return this value anywhere user-visible."""
    v = os.environ.get(name)
    if v:
        return v
    pat = re.compile(
        r"^[ \t]*(?:export[ \t]+)?" + re.escape(name) + r"[ \t]*=[ \t]*(.*)$", re.M
    )
    for f in SHELL_ENV_FILES:
...
def litrev_contact() -> str | None:
    ...
    Env var first, then git's configured user.email; None if unavailable."""
    for k in ("LITREV_CONTACT_EMAIL", "CROSSREF_MAILTO", "EMAIL"):
        v = litrev_env(k)
        if v and "@" in v:
            return v
    try:
...
def litrev_openalex_key() -> str:
    ...
    Resolution order: ``OPENALEX_API_KEY`` in the environment, then a parse of
    ``~/.shell_env`` / ``~/.env``. Raises RuntimeError with actionable guidance
    when no key is available — callers must NOT fall back to anonymous calls."""
    k = litrev_env("OPENALEX_API_KEY")
    if k:
        return k
    raise RuntimeError(
        "OpenAlex requires a free API key and none was found. Skip "
        "OpenAlex-backed steps (search_openalex / expand_citations); keyless "
        "calls are unsupported. Fix: put the key in ~/.shell_env as "
        "`export OPENALEX_API_KEY=...` (the `export` is required — without it "
        "the value stays a shell-local variable and never reaches Python), and "
        'make sure ~/.claude/settings.json has "BASH_ENV": "/home/<user>/.shell_env" '
        "in its env block. Get a free key at https://openalex.org/settings/api."
    )
