...
| Similarity/substructure search (purchasable compounds) | PubChem | ChEMBL |
| FDA-approved drug structures | PubChem | FDA |
| Commercial vendor catalogs / compound purchasability | *(no working option)* | ZINC was removed 2026-07-15 — verified CAPTCHA-blocked on all endpoints, no open API remains |
...
| User is asking about... | Primary database(s) | Also consider |
|---|---|---|
| Somatic mutations in cancer | Open Targets | cBioPortal |
| Cancer genomics (TCGA) | GDC (TCGA) | cBioPortal |
| Cancer study mutations, CNA, expression | cBioPortal | GDC (TCGA) |
| Tumor clinical data (survival, staging) | cBioPortal | GDC (TCGA) |
| Drug-target-disease associations | Open Targets | ChEMBL |
| GWAS/L2G/fine-mapping genetics evidence | `opentargets-database` *skill* (not this reference) | — |
...
| User is asking about... | Primary database(s) | Also consider |
|---|---|---|
| Everything about a compound | PubChem + ChEMBL + DrugBank | BindingDB, Reactome, FDA |
| Everything about a gene | NCBI Gene + UniProt + Ensembl | Reactome, STRING, cBioPortal, ENA |
| Everything about a variant | dbSNP + ClinVar + gnomAD | GWAS Catalog, cBioPortal |
...
| COSMIC | No REST/JSON API exists (verified 2026-07-15) — licensed bulk flat-file download only, no live query endpoint | Use **Open Targets** (ingests COSMIC's Cancer Gene Census) or **cBioPortal**/**GDC** for per-sample data |
| BRENDA | Free registration required (SOAP, not REST) | Use **KEGG** for enzyme/pathway data |
| ZINC | No open API (verified 2026-07-15) — all endpoints CAPTCHA-gated, bulk mirror requires an account | Use **PubChem** for purchasability/similarity search |
...
- **Bound total work**: For broad searches, start with a count or first page. Do not continue past 10,000 records or 100 API calls without explicit user confirmation and a short retrieval plan. For very large sources such as PubChem, ChEMBL, SEC archives, or bulk genomics repositories, prefer official bulk downloads or database dumps when the user truly needs all records.
...
- **Page number**: `page=1&per_page=50` → increment page (World Bank, cBioPortal)
...
| Database | Reference File | What it covers |
|---|---|---|
| Open Targets | `references/opentargets.md` | Target-disease associations (POST); also the best free source for somatic cancer mutations (COSMIC has no working API — see restricted-access table) |
