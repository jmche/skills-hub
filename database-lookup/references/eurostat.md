...
| Parameter | Required | Description |
|-----------|----------|-------------|
| `format` | No | `json` (default; the documented `sdmx+json` value returns HTTP 406 "UNSUPPORTED_FORMAT" -- verified), `sdmx-csv` (not `sdmx+csv`, also 406 -- verified), `sdmx+xml`, `TSV` |
...
```
https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/prc_hicp_mmor/M.RCH_A.CP00.DE+FR+IT?startPeriod=2023-01&endPeriod=2024-06&format=json
```

### 2. Get Dataset as CSV

Append `?format=sdmx-csv` (hyphen, not `+`) to any data request for a flat CSV response that is easier to parse.
